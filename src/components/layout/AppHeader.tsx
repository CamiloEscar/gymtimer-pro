    ...
    <header className="sticky top-0 z-10 bg-surface-900 border-b border-surface-800 px-4 py-3">
      <nav className="flex flex-wrap items-center justify-between gap-x-2 gap-y-2 md:gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <Link href="/app" className="font-industrial text-white tracking-widest text-sm md:text-base">
            GYMTIMER
          </Link>
        </div>
        <ul className="flex items-center gap-2 md:gap-4 text-xs md:text-sm font-tactical">
          {SECTION_LINKS.map((link) => {
            ...
            const className = active ? "text-brand-500" : "text-gray-400 hover:text-white";
            return (
              <li key={link.href}>
