...
describe("RoundIndicator", () => {
  it("uses brand color on the last round and phosphor-dim otherwise", () => {
    const { rerender } = render(<RoundIndicator round={3} totalRounds={5} />);
    const mid = screen.getByText("[ RONDA 3 / 5 ]");
    expect(mid.className).toContain("text-phosphor-dim");
    ...
    expect(last.className).not.toContain("text-phosphor-dim");
  });
});
