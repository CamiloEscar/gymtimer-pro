...
describe("AppHeader", () => {
  ...
  it("renders the five section links with correct hrefs", () => {
    mockPathname("/app");
    render(<AppHeader />);
  ...
  it("renders the back button on /app/exercises/leg-01 pointing to /app/exercises", () => {
    mockPathname("/app/exercises/leg-01");
    render(<AppHeader />);
    const back = screen.getByRole("link", { name: "Atrás" });
    expect(back).toHaveAttribute("href", "/app/exercises");
  });
  ...
  it("highlights the Rutinas section link on /app/workouts", () => {
    ...
    expect(screen.getByRole("link", { name: "Dashboard" })).toHaveClass("text-phosphor-dim");
  });
  ...
  it("highlights the Dashboard section link on /app", () => {
    ...
    expect(screen.getByRole("link", { name: "Rutinas" })).toHaveClass("text-phosphor-dim");
  });
